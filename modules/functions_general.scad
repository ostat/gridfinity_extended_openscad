...
/*
defaultDemoSetting = [3,2,"base","default","",false,"",false,false];
x = [[0,-1],[8,-2]];

echo(len=len(defaultDemoSetting),defaultDemoSetting=defaultDemoSetting);


arr = defaultDemoSetting;
keyValueArray = x;
...
//partial
echo(partial=position < len(list)-1 ? partial(list,start=position+1,end=len(list)-1) : []);

//replace
echo(replace=replace(arr, 0, 8));
l1 = position > 0 ? partial(list,start=0,end=position-1) : [];
echo(l1=l1);
l2 = partial(list,start=position+1,end=len(list)-1);
echo(l2=l2);

//replace_Items
currentKeyValue = keyValueArray[0];
echo(currentKeyValue=currentKeyValue);
keyValueArrayNext = remove_item(keyValueArray,0);
echo(keyValueArrayNext=keyValueArrayNext);
updatedList = replace(arr, currentKeyValue[1],currentKeyValue[0]);

echo(updatedList=updatedList);
echo(replace_Items(x,defaultDemoSetting));
*/
