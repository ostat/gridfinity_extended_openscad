    ...
    difference() {
        union() {
            gridfinity_baseplate(
                num_x = plate[1].x[iPlate_size],
                num_y = plate[1].y[iPlate_size],
                outer_num_x = plate[1].x[iPlate_outerSize],
                outer_num_y = plate[1].y[iPlate_outerSize],
                outer_height = outer_Height,
                position_fill_grid_x = plate[1].x[iPlate_posGrid],
                position_fill_grid_y = plate[1].y[iPlate_posGrid],
                position_grid_in_outer_x = plate[1].x[iPlate_posOuter],
                position_grid_in_outer_y = plate[1].y[iPlate_posOuter],
            ...
            if (enable_snapfinity) { // Global customizer parameter
                generate_all_baseplate_tabs(
                    num_x_units = plate[1].x[iPlate_size], // Current plate's X grid units
                    num_y_units = plate[1].y[iPlate_size], // Current plate's Y grid units
                    snapfinity_tab_width = snapfinity_tab_width, // Global customizer parameter
                    actual_pitch_x = pitch[0], // Global customizer parameter 'pitch'
                    actual_pitch_y = pitch[1]  // Global customizer parameter 'pitch'
                );
            }
        } // End union
        
        if (enable_snapfinity) { // Global customizer parameter
            generate_all_baseplate_cutaways(
                num_x_units = plate[1].x[iPlate_size], // Current plate's X grid units
                num_y_units = plate[1].y[iPlate_size], // Current plate's Y grid units
                snapfinity_tab_width = snapfinity_tab_width, // Global customizer parameter
                actual_pitch_x = pitch[0], // Global customizer parameter 'pitch'
